...
2. 域名（如 `www.chaomifilter.com`）添加 CNAME 解析到 Bucket 外网域名：
   ```
   www.chaomifilter.com CNAME ultraseek-website.oss-cn-hangzhou.aliyuncs.com
   ```
...
3. 加速类型：图片小文件
4. 配置回源协议 HTTPS
5. 修改 DNS：把 `www.chaomifilter.com` CNAME 到 CDN 加速域名
...
- 用 Let's Encrypt 免费证书：`certbot --nginx -d www.chaomifilter.com`
...
**Q: 自定义域名访问报错？**
A: 检查 CNAME 解析是否生效，用 `dig www.chaomifilter.com` 验证。
